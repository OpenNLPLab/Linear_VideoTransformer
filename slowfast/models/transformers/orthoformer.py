...
def orthoformer(
    ...
    if shared_landmarks:
        ...
        kernel_2 = Fn.softmax(torch.matmul(
                landmarks, k.transpose(-1, -2)), dim=-1)
        # v = rearrange(v, 'b (f p) d -> b f p d', f=F)
        x = torch.einsum('b l p, b p d -> b l d', kernel_2, v)
        x = torch.einsum('b n l, b l d -> b n d', kernel_1, x)
        if return_attn:
            attn = torch.einsum('b m l, b l p -> b m p', kernel_1, kernel_2)
            return x, attn
