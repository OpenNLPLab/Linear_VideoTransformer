...
from .vit_helperv2 import Block, CrossAttnBlock
...
class PredictorLG(nn.Module):
    ...
    def forward(self, x, policy):
        ...
        global_x = x[:,:, C//2:].mean(1, keepdim=True)
...
class Tokenfuse(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, in_dim=3136, out_dim=784):
        super().__init__()
        self.in_conv = nn.Sequential(
            nn.LayerNorm(in_dim),
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        base_rate = [1, 0.7, 0.5, 0.3]
        pruning_loc = [1, 3, 5]
        self.pruning_loc = pruning_loc
        self.token_ratio = base_rate
        #predictor_list = [PredictorLG(embed_dim) for _ in range(len(pruning_loc))]
        tokenfuse_list = [Tokenfuse(in_dim=int(self.token_ratio[i]*3136), out_dim=int(self.token_ratio[i+1]*3136)) for i, _ in enumerate(pruning_loc)]
        #self.score_predictor = nn.ModuleList(predictor_list)
        self.token_fuser = nn.ModuleList(tokenfuse_list)
    ...
    def index_select(self, x, logits, count):
        B = x.shape[0]
        keep_index = torch.argsort(logits, dim=1, descending=True)[:, :count]
        #index = torch.topk(logits, count, largest=True, dim=1)
        cls_idx = torch.zeros(B, 1, dtype=keep_index.dtype, device=keep_index.device)
        now_policy = torch.cat([cls_idx, keep_index + 1], dim=1)
        #index = torch.cat([cls_idx, index+1], dim=1).unsqueeze(-1).repeat(1, 1, x.shape[-1])
        #y = torch.gather(x, 1, index)
        x = batch_index_select(x, now_policy)
        return x

    def hard_softmax(self, logits, count):
        y_soft = logits
        # Straight through.
    ...
    def forward_features(self, x, num_frames=None, filename=None):
        ...
        x = rearrange(x, '(b f) n e -> b (f n) e', f=num_frames)
        B = x.shape[0]
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.pruning_loc:
                cls_token = x[:, 0]
                spatial_x = x[:, 1:]
                #prev_decision = torch.ones(B, spatial_x.shape[1], 1, dtype=x.dtype, device=x.device)
                #pred_score = self.score_predictor[p_count](spatial_x, prev_decision).reshape(B, -1)
                #count = int(self.token_ratio[p_count]*spatial_x.shape[1])
                #x = self.hard_softmax(pred_score, count)
                #x = self.index_select(x, pred_score, count)
                spatial_x = self.token_fuser[p_count](spatial_x)
...
@register_model
def vit_prunev2_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def vit_prunev2_base_patch16_224_in21k(pretrained=False, **kwargs):
...
@register_model
def vit_prunev2_base_patch16_224_in21k_debug(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(
...
@register_model
def vit_prunev2_base_patch32_224_in21k(pretrained=False, **kwargs):
...
@register_model
def vit_prunev2_large_patch16_224_in21k(pretrained=False, **kwargs):
...
@register_model
def vit_prunev2_large_patch32_224_in21k(pretrained=False, **kwargs):
...
@register_model
def vit_prunev2_base_patch32_384(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
...
@register_model
def vit_prunev2_base_patch16_384(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(
