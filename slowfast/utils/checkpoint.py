...
def get_path_to_checkpoint(path_to_job, epoch, use_epoch=True, extra=""):
    """
    ...
    """
    if use_epoch:
        name = "{}checkpoint_epoch_{:05d}.pyth".format(extra, epoch)
    else:
        name = "{}checkpoint_epoch_{:05d}.pyth".format(extra, 1)
...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, is_best=False):
    """
    Save a checkpoint.
    Args:
...
def load_checkpoint(
    ...
    try:
        ...
        checkpoint = new_checkpoint_dict
    ...
    if inflation:
        ...
        pre_train_dict = (
            checkpoint
            if not "model_state" in checkpoint
    ...
    if "epoch" in checkpoint.keys() and not epoch_reset:
        ...
        if optimizer:
            try:
                ...
                print("Loading the optimizer failed")
    else:
        epoch = -1
    return epoch
